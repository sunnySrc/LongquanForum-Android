...
public class EmotiomComplateFragment extends BaseFragment {
    ...
    private void initEmotion() {
        ...
        //动态计算gridview的总高度
        int gvHeight = itemWidth * 4 + spacing * 5;

        List<GridView> emotionViews = new ArrayList<>();
        List<String> emotionNames = new ArrayList<>();
        // 遍历所有的表情的key
        Iterator it = EmotionUtils.getEmojiMap(emotion_map_type).iterator();
        while (it.hasNext()) {
            EmotionUtils.Item item = (EmotionUtils.Item)it.next();
            emotionNames.add(item.mString);
            // 每20个表情作为一组,同时添加到ViewPager对应的view集合中
            if (emotionNames.size() == 28) {
